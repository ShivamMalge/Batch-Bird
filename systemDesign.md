...
`Utf8Dict { dict: Arc<[String]>, codes: Vec<u32> }`. Group-by hashes `u32` codes, not raw
strings — cheap integer hashing/equality instead of string hashing per row.

**The dictionary is shared, not owned per column** (amended 2026-09-04 with sign-off; the
original pinned type was `Vec<String>`). `RecordBatch` owns its `Column`s, so with an owned
dictionary every batch cloned the whole thing — once in `Scan`, again in `Filter` compaction.
Measured over 2M rows: at 10k distinct values the batch pipeline ran **84× slower** than the
row-at-a-time baseline, while an `Int64` group column over identical data (same cardinality,
same slot count, same scatter pattern) stayed at 1.1×. The benchmark would have been reporting
allocator throughput. With `Arc` the Utf8 path tracks the Int64 path at ~0.70× across every
cardinality tested. Arrow and DuckDB share dictionaries for the same reason.
`examples/smoke.rs` reproduces the measurement.
