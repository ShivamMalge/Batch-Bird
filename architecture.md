...
enum Column {
    Int64(Vec<i64>),
    Float64(Vec<f64>),
    // Amended 2026-09-04 with sign-off: the dictionary was a plain Vec<String>, which made
    // every RecordBatch clone it (twice — Scan and Filter compaction). Measured 84x slower
    // than the naive baseline at 10k distinct values; Arc makes it a refcount bump and the
    // ratio returns to ~0.70x. See systemDesign.md "String Columns".
    Utf8Dict { dict: Arc<[String]>, codes: Vec<u32> },
}
