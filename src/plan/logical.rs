...
impl Aggregation {
    /// The column name this aggregate produces in the result, matching how SQL labels it.
    ///
    /// Lives here rather than in either engine because a query's output schema is a property
    /// of the *query*, not of the strategy used to execute it. Both the naive baseline and the
    /// batch pipeline label their results with this, which is what lets the parity test
    /// compare them by name.
    pub fn output_name(&self) -> String {
        match self.func {
            AggFunc::Sum => format!("SUM({})", self.input),
        }
    }
}
